/*
 ...
 =============================================================================
 History:
 Author: Miroslaw Dach SLS/PSI 16.10.2004
 -----------------------------------------------------------------------------
 -----------------------------------------------------------------------------
    ...
    struct stringinRecord    *pStringIn;
{
  /*     long status; */
    ...
    else if(!strcmp(pvmeio->parm,"BOOTIME"))
    {
	 ...
	 iUpTime=get_uptime();
	 iCurTime=iCurTime-(time_t)iUpTime;
...
/*-------------- get IP address ---------------------------*/
int myIpAddr(char* buffer)
{
  int s;
  struct ifreq ifr;

  strcpy(buffer,"*NO IP ACQUIRED*");
  if( (s=socket(AF_INET,SOCK_DGRAM,0)) < 0) return -1;
  strcpy(ifr.ifr_name,"eth0");
  if( ioctl(s,SIOCGIFADDR,&ifr)) return -1;
...
int myMacAddr(char* buffer)
{
  ...
  if( (s=socket(AF_INET,SOCK_DGRAM,0)) < 0) return -1;
  strcpy(ifr.ifr_name,"eth0");
  if( ioctl(s,SIOCGIFHWADDR,&ifr)) return -1;

  dat = (char*)&ifr.ifr_hwaddr.sa_data;
  sprintf(buffer,"%2.2X:%2.2X:%2.2X:%2.2X:%2.2X:%2.2X",
     (int)(*(dat+0)&0xFF),(int)(*(dat+1)&0xFF),(int)(*(dat+2)&0xFF),
     (int)(*(dat+3)&0xFF),(int)(*(dat+4)&0xFF),(int)(*(dat+5)&0xFF));
...
unsigned long myMask(void)
{
  ...
  if( (s=socket(AF_INET,SOCK_DGRAM,0)) < 0) return -1;
  strcpy(ifr.ifr_name,"eth0");
  if( ioctl(s,SIOCGIFNETMASK,&ifr)) return -1;
...
/*-------------- EPICS Version dset ---------------------------*/
static long init_record_EpicsVer();
static long read_EpicsVer();
struct {
        long            number;
        DEVSUPFUN       report;
        DEVSUPFUN       init;
        ...
        NULL,
        NULL,
        init_record_EpicsVer,
        NULL,
        read_EpicsVer,
};
epicsExportAddress(dset,devEpicsVer);

static long init_record_EpicsVer(struct stringinRecord* pStringIn)
{
    if(recGblInitConstantLink(&pStringIn->inp,DBF_STRING,&pStringIn->val)) pStringIn->udf = FALSE;
    return(0);
}

static long read_EpicsVer(struct stringinRecord* pStringIn)
{
    strcpy(pStringIn->val,EPICS_VERSION_STRING);
    return( 0 );
}
...
static long init_record_IP(struct stringinRecord* pStringIn)
{
    if(recGblInitConstantLink(&pStringIn->inp,DBF_STRING,&pStringIn->val)) pStringIn->udf = FALSE;
    return(0);
}

static long read_uptime_IP(struct stringinRecord* pStringIn)
{
    char buffer[1024];

    myIpAddr(buffer);
    strcpy(pStringIn->val,buffer);
    return( 0 );
}


/*-------------- MAC Address dset ---------------------------*/
static long init_record_MAC();
static long read_uptime_MAC();
struct {
        long            number;
        DEVSUPFUN       report;
        DEVSUPFUN       init;
...
epicsExportAddress(dset,devMacAddr);

static long init_record_MAC(struct stringinRecord* pStringIn)
{
    if(recGblInitConstantLink(&pStringIn->inp,DBF_STRING,&pStringIn->val)) pStringIn->udf = FALSE;
    return(0);
}

static long read_uptime_MAC(struct stringinRecord* pStringIn)
{
    char buffer[1024];
...
/*-------------- Network Mask dset ---------------------------*/
static long init_record_NetMask();
static long read_uptime_NetMask();
struct {
        long            number;
        DEVSUPFUN       report;
        DEVSUPFUN       init;
...
epicsExportAddress(dset,devNetMask);

static long init_record_NetMask(struct stringinRecord* pStringIn)
{
    if(recGblInitConstantLink(&pStringIn->inp,DBF_STRING,&pStringIn->val)) pStringIn->udf = FALSE;
    return(0);
}

static long read_uptime_NetMask(struct stringinRecord* pStringIn)
{
    long mask=0;

    mask = myMask();
    if( mask == -1 )
        strcpy(pStringIn->val,"*NO MASK ACQUIRED*");
    else
        sprintf(pStringIn->val,"%lu:%lu:%lu:%lu",mask%256,(mask/256)%256,(mask/65536)%256,(mask/16777216)%256);
    ...
    struct stringinRecord    *pStringIn;
{
    ...
    else
    {
	    ...
	    else if(!strcmp(pvmeio->parm,"NODENAME"))
	        sprintf(pStringIn->val,"%s",name.nodename);
