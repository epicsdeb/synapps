...
int BSmakeServer(char** argv)
{
    ...
    switch(fork())
    {
        case -1: /* error */
			...
			return -1;
        case 0: /* child */
#if defined linux || defined SOLARIS || defined SGI || defined HP_UX || defined CYGWIN32
			setpgrp();
#elif defined darwin
            setpgid(0,0);
#else
...
#else
#define IOCTL_TYPE void *
#endif

	ifc.ifc_len = sizeof(buf);
	ifc.ifc_buf = buf;
	if(ioctl(soc,SIOCGIFCONF,(IOCTL_TYPE)&ifc) < 0)
	...
	do
	{
		...
		{
			if(ioctl(soc,SIOCGIFFLAGS,(IOCTL_TYPE)&ifr[i])<0)
	...
	if(save)
	{
		if(ioctl(soc,SIOCGIFBRDADDR,(IOCTL_TYPE)save)<0)
