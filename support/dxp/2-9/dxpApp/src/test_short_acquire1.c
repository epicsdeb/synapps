    ...
    for (iter=0; iter<MAX_ITERATIONS; iter++) {
        ...
        for (check=0; check<MAX_CHECKS; check++) {
            ...
            for (chan=0; chan<NUM_CHANNELS; chan++) {
                ...
                if (((acquiring[chan] & XIA_RUN_HARDWARE) == 0) && (check == 0)) {
                    /* Something is wrong - the module should not be done the first time we check it */
                    printf("Error: channel %d is done on first check\n", chan);
                }
            }
            for (chan=0; chan<NUM_CHANNELS; chan++) {
