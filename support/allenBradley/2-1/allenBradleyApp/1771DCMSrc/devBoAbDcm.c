...
static long bo_write();
struct {
	long		number;
	DEVSUPFUN	dev_report;
	DEVSUPFUN	dev_init;
	DEVSUPFUN	dev_init_record;
	DEVSUPFUN	bo_get_ioint_info;
	DEVSUPFUN	bo_write;
}devBoAbDcm={
	6,
	NULL,
	...
	NULL,
	bo_write};
epicsExportAddress(dset,devBoAbDcm);
...
static long bo_init(int after)
{
	...
	if(rtn!=OK) errlogPrintf("devBoAbDcm: cant locate abDcmTable\n");
    }
    return(0);
}

static long bo_init_record(struct boRecord *precord)
{
...
{
    boRecord *precord = (boRecord *)userPvt;
...
static long bo_write(struct boRecord *precord)
{
