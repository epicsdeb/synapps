...
struct {
	...
	DEVSUPFUN	mbbo_get_ioint_info;
	DEVSUPFUN	mbbo_write;
}devMbboAbDcm={
	6,
	NULL,
	mbbo_init,
	...
	NULL,
	mbbo_write};
epicsExportAddress(dset,devMbboAbDcm);
...

static long mbbo_init(int after)
{
    if(!after) {
	STATUS		rtn;
	SYM_TYPE	type;
	...
	if(rtn!=OK) errlogPrintf("devMbboAbDcm: cant locate abDcmTable\n");
    }
    return(0);
}

static long mbbo_init_record(struct mbboRecord *precord)
{
...
{
    mbboRecord *precord = (mbboRecord *)userPvt;
...
static long mbbo_write(struct mbboRecord *precord)
{
    ...
    dcmStatus = (*pabDcm->put_ushort)(pdevicePvt->pabDcmPvt,
	pdevicePvt->tag,(unsigned short)(precord->mask),
	(unsigned short)precord->rval,myCallback,(void *)precord);
    if(dcmStatus!=abDcmOK) {
